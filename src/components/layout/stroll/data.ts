...
const recommendDetails: RecommendDetailType[] = [
  ...
  },
  {
    ...
    description_html:
      '从组件生成、变量联动到设计规范自动补全，看看 AI 是怎么进入设计系统流程的。',
    content_html: `
      <h2 id="overview">内容概览</h2>
...
export async function getRecommendDetailById(id: string): Promise<RecommendDetailType | null> {
  await new Promise((resolve) => setTimeout(resolve, 50));
  return recommendDetails.find((item) => item.id === id) ?? null;
}
