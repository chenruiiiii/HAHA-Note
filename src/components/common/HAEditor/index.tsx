...
const HAEditor = ({
  ...
  onChange,
  onSave,
  saveStatusText: externalSaveStatusText,
}: HAEditorProps) => {
  const [title, setTitle] = useState(initialTitle);
  const [outlineItems, setOutlineItems] = useState<HAEditorOutlineItem[]>([]);
  const [internalSaveStatusText, setInternalSaveStatusText] = useState(getInitialSaveStatusText(initialSavedAt));
  // 外部传入的 saveStatusText 优先（版本化自动保存状态指示器），否则用内部状态
  const saveStatusText = externalSaveStatusText ?? internalSaveStatusText;
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setInternalSaveStatusText(getSaveStatusText());
    } catch {
      const duration = Math.round(now() - saveStartedAt);
      trackPerformance('note_save_completed', {
        metric_name: 'save_note_ms',
  ...
  useEffect(() => {
    setInternalSaveStatusText(getInitialSaveStatusText(initialSavedAt));
  }, [initialSavedAt]);
